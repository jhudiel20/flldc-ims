...
$dbfield = array('daily_inventory.ID as ID','daily_inventory.PRODUCT_CODE as PRODUCT_CODE','PHYSICAL_COUNT','BEGINNING','ENDING'); // need iset based sa table columns na need sa query
$db_orig = array('ID','PRODUCT_CODE','BEGINNING','ENDING','PHYSICAL_COUNT','DATE_CREATED'); //gamit sa filter  tabulator
...
array_push($sql_where_array, " DATE(daily_inventory.DATE_CREATED) = CURDATE()"); //set default WHERE CONDITION HERE
...
$default_query ="SELECT ".$field_query." FROM daily_inventory ".$sql_conds;
...
$default_query ="SELECT ".$field_query.",PRODUCT_NAME ,DATE_FORMAT(daily_inventory.DATE_CREATED, '%d-%M-%Y') as DATE_CREATED 
FROM daily_inventory JOIN product on product.PRODUCT_CODE = daily_inventory.PRODUCT_CODE ".$sql_conds." ORDER BY daily_inventory.DATE_CREATED DESC " ;
