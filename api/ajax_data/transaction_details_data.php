...
$TRANSACTION_ID = mysqli_real_escape_string($conn, isset($_POST['TRANSACTION_ID']) ? trim($_POST['TRANSACTION_ID']) : '');
...
$dbfield = array('t.TRANSACTION_ID','SUBTOTAL','LESSVAT','NETVAT','ADDVAT','GRANDTOTAL','CASH','DATE','td.PRODUCTS','QTY','CUSTOMER_NAME','PRODUCT_CODE','PRICE','td.DATE_CREATED'); // need iset based sa table columns na need sa query
$db_orig = array('TRANSACTION_ID','SUBTOTAL','LESSVAT','NETVAT','ADDVAT','GRANDTOTAL','CASH','DATE','td.PRODUCTS','QTY','CUSTOMER_NAME','PRODUCT_CODE','PRICE','DATE_CREATED'); //gamit sa filter  tabulator
...
array_push($sql_where_array, " t.TRANSACTION_ID = '$TRANSACTION_ID' "); //set default WHERE CONDITION HERE
...
//total query counter 
$total_query = 0;
$field_query ='COUNT(DISTINCT t.ID) AS count'; // baguhin based sa need
$sql_conds = (empty($sql_where)) ? '' : 'WHERE '.$sql_where;

$default_query ="SELECT ".$field_query." FROM transaction as t
JOIN transaction_details as td on td.TRANSACTION_ID = t.TRANSACTION_ID
".$sql_conds;
...
$default_query ="SELECT * FROM transaction as t
JOIN transaction_details as td on td.TRANSACTION_ID = t.TRANSACTION_ID
".$sql_conds;
