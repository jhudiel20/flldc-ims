...
$CODE = $_GET['CODE'];
// $transacID = mysqli_real_escape_string($conn, isset($_POST['transac_ID']) ? trim($_POST['transac_ID']) : '');
...
$dbfield = array('BEGINNING','ENDING','ID','PHYSICAL_COUNT', 'DATE_CREATED'); // need iset based sa table columns na need sa query
$db_orig = array('BEGINNING','ENDING','ID','PHYSICAL_COUNT', 'MONTH','YEAR'); //gamit sa filter  tabulator
...
array_push($sql_where_array, " PRODUCT_CODE = '$CODE' AND DATE(monthly_inventory.DATE_CREATED) <= DATE_FORMAT(CURDATE() ,'%Y-%m-01')"); //set default WHERE CONDITION HERE
...
$default_query ="SELECT ".$field_query." FROM monthly_inventory ".$sql_conds;
...
$default_query ="SELECT ".$field_query.",DATE_FORMAT(monthly_inventory.DATE_CREATED, '%Y') as YEAR, DATE_FORMAT(monthly_inventory.DATE_CREATED, '%M') as MONTH
FROM monthly_inventory ".$sql_conds." ORDER BY ".$orderby ;
