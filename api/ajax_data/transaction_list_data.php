...
$dbfield = array('ID','PURPOSE','TRANSACTION_ID','CREATED_BY','TRANSACTION_DATE_CREATED'); // need iset based sa table columns na need sa query
$db_orig = array('ID','PURPOSE','TRANSACTION_ID','CREATED_BY','TRANSACTION_DATE_CREATED'); //gamit sa filter  tabulator
...
$default_query ="SELECT ".$field_query.", DATE_FORMAT(TRANSACTION_DATE_CREATED, '%Y-%m-%d %h:%i:%s %p') as TRANSACTION_DATE_CREATED 
FROM transaction ".$sql_conds." ORDER BY TRANSACTION_DATE_CREATED DESC";
...
if($query = mysqli_query($conn,$sql_limit)){
	if($num = mysqli_num_rows($query)){
		while($data = mysqli_fetch_assoc($query)){
			$class_text ="";
			$data['xid'] = encrypted_string($data['TRANSACTION_ID']);
