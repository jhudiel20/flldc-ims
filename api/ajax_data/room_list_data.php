...
$query_fields = ['id', 'room_name', 'room_id', 'room_type','capacity','floor_number','status'];

if (!empty($sorters)) {
    $valid_sorts = ['id', 'room_name', 'room_id', 'room_type','capacity','floor_number','status','date_created'];
...
$data_query = "SELECT $select_fields, TO_CHAR(date_created, 'YYYY-MM-DD HH12:MI:SS AM') as date_created
                FROM room_details $filter_sql ORDER BY $sort_field $sort_dir
