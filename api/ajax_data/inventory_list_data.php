...
$dbfield = array('PRODUCT_CODE','SUPPLIER_NAME','PRODUCT_NAME','BRAND_NAME','CATEGORY_NAME','SUBCATEGORY_NAME','TABLE_ID','stock.ID','QUANTITY', 'MEASUREMENT', 'EXPIRATION', 'REMARKS'); // need iset based sa table columns na need sa query
$db_orig = array('PRODUCT_CODE','SUPPLIER_NAME','PRODUCT_NAME','BRAND_NAME','CATEGORY_NAME','SUBCATEGORY_NAME','TABLE_ID','ID','QUANTITY', 'MEASUREMENT', 'EXPIRATION', 'REMARKS'); //gamit sa filter  tabulator
...
// array_push($sql_where_array, " ID = '$product_id'"); //set default WHERE CONDITION HERE
...
//total query counter 
$total_query = 0;
$field_query ='COUNT(DISTINCT TABLE_ID) AS count'; // baguhin based sa need
$sql_conds = (empty($sql_where)) ? '' : 'WHERE '.$sql_where;

$default_query ="SELECT ".$field_query." FROM stock 
JOIN product ON product.PRODUCT_CODE = stock.ID
".$sql_conds;
