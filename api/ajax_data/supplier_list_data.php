...
$dbfield = array('SUPPLIER_ID','COMPANY_NAME','ADDRESS','PHONE_NUMBER','CREATED_BY','DATE_CREATED'); // need iset based sa table columns na need sa query
$db_orig = array('SUPPLIER_ID','COMPANY_NAME','ADDRESS','PHONE_NUMBER','CREATED_BY','DATE_CREATED'); //gamit sa filter  tabulator
...
//total query counter 
$total_query = 0;
$field_query ='COUNT(DISTINCT SUPPLIER_ID) as count'; // baguhin based sa need
$sql_conds = (empty($sql_where)) ? '' : 'WHERE '.$sql_where;
$default_query ="SELECT ".$field_query." FROM supplier ".$sql_conds;
...
$default_query ="SELECT ".$field_query.",DATE_FORMAT(DATE_CREATED, '%Y-%m-%d %h:%i:%s %p') as DATE_CREATED FROM supplier ".$sql_conds." ORDER BY ".$orderby;
