...
$dbfield = array('PRODUCT_CODE','PRODUCT_NAME', 'BRAND_NAME', 'CATEGORY_NAME', 'SUBCATEGORY_NAME', 'SUPPLIER_NAME','STATUS','SPECS','TYPE'); // need iset based sa table columns na need sa query
$db_orig = array('ID','PRODUCT_CODE','PRODUCT_NAME', 'BRAND_NAME', 'PRICE', 'CATEGORY_NAME', 'SUBCATEGORY_NAME', 'SUPPLIER_NAME' , 'DATE_CREATED','STATUS','SPECS','TYPE'); //gamit sa filter  tabulator
...
array_push($sql_where_array, " STATUS = 'Disapproved' "); //set default WHERE CONDITION HERE
...
$default_query ="SELECT ".$field_query." FROM product ".$sql_conds;
...
$default_query ="SELECT ".$field_query.",QUANTITY,product.ID, DATE_FORMAT(product.DATE_CREATED, '%d-%m-%Y') as DATE_CREATED, 
FORMAT(PRICE, 2) AS PRICE FROM product 
JOIN stock on stock.ID = PRODUCT_CODE	
".$sql_conds. "ORDER BY CASE 
        WHEN stock.QUANTITY <= 20 THEN 0 
        ELSE 1 
...
if($query = mysqli_query($conn,$sql_limit)){
	if($num = mysqli_num_rows($query)){
		while($data = mysqli_fetch_assoc($query)){
			$class_text ="";
			$data['xid'] = encrypted_string($data['PRODUCT_CODE']);
