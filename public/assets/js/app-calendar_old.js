...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    const calendarEl = document.getElementById('calendar'),
      ...
      btnUpdate = document.querySelector('button[type="submit"][id="update_calendar_events"]'),
    ...
    function fetchEvents(info, successCallback) {
      ...
      $.ajax({
        url: '../assets/js/ajax/app-calendar-events.php',
        type: 'GET',
        success: function(result) {
          console.log('Fetched events:', result.events);
          
          // Pass the fetched events to the success callback
    ...
    function addEvent(eventData) {
      ...
      // calendar.addEvent(eventData);
      $.ajax({
        url: '../action/add-events.php',
        type: 'POST',
        data: eventData,
        success: function(response) {
          console.log('Event updated successfully:', response);
          // If the server-side update was successful, update the event in the currentEvents array
         
            Toast.fire({
              icon: 'success',
              title: 'Success',
              text: 'Event Added uccessfully!'
            })
            // .then(function() {
            //   location.reload();
            // });
    ...
    // }
    function updateEvent(eventData) {
      // Send the updated event data to the server for updating
      $.ajax({
        url: '../action/update-events.php',
        type: 'POST',
        data: eventData,
        success: function(response) {
          console.log('Event updated successfully:', response);
    ...
    function removeEvent(eventId) {
      ...
      $.ajax({
        url: '../action/remove-events.php',
        type: 'POST',
        data: { eventId: eventId }, // Send the event ID to identify which event to delete
        success: function(response) {
          // On successful deletion from the server, remove the event from the calendar
          currentEvents = currentEvents.filter(function (event) {
            return event.id != eventId;
          });
          calendar.refetchEvents();
    ...
    btnSubmit.addEventListener('click', e => {
      if (btnSubmit.classList.contains('btn-add-event')) {
        if (isFormValid) {
          // let allDayValue = allDaySwitch.checked ? true : false; 
          let newEvent = {
          ...
          if (allDaySwitch.checked) {
            newEvent.allDay = true; // Set allDay to true when checked
          } else {
              newEvent.allDay = false; // Set allDay to false when not checked
          }
    ...
    btnUpdate.addEventListener('click', e => {
        if (btnUpdate.classList.contains('btn-update-event')) {
        // Update event
        // ------------------------------------------------
        if (isFormValid) {
          let eventData = {
          ...
          console.log(allDaySwitch.checked);
          if (allDaySwitch.checked) {
            eventData.allDay = true; // Set allDay to true when checked
          } else {
            eventData.allDay = false; // Set allDay to false when not checked
          }
