...
$dbfield = array('CATEGORY_ID','CAT_CODE','NAME','REMARKS','ADDED_BY','INFO','DATE_CREATED'); // need iset based sa table columns na need sa query
$db_orig = array('CATEGORY_ID','CAT_CODE','NAME','REMARKS','ADDED_BY','INFO','DATE_CREATED'); //gamit sa filter  tabulator
...
//total query counter 
$total_query = 0;
$field_query ='COUNT(DISTINCT CATEGORY_ID) as count'; // baguhin based sa need
$sql_conds = (empty($sql_where)) ? '' : 'WHERE '.$sql_where;
$default_query ="SELECT ".$field_query." FROM category ".$sql_conds;
...
$default_query ="SELECT ".$field_query." FROM category ".$sql_conds." ORDER BY ".$orderby;
