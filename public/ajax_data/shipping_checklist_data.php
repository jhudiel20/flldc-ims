...
$dbfield = array('ID','TRANSACTION_ID','SUBTOTAL','LESSVAT','NETVAT','ADDVAT','GRANDTOTAL','CASH','DATE','TRANSACTION_STATUS','CUSTOMER_NAME','DELIVERED_DATE'); // need iset based sa table columns na need sa query
$db_orig = array('ID','TRANSACTION_ID','SUBTOTAL','LESSVAT','NETVAT','ADDVAT','GRANDTOTAL','CASH','DATE','TRANSACTION_STATUS','CUSTOMER_NAME','TRANSACTION_DATE_CREATED','DELIVERED_DATE'); //gamit sa filter  tabulator
...
array_push($sql_where_array, " TRANSACTION_STATUS = 'Pending'  "); //set default WHERE CONDITION HERE
...
$default_query ="SELECT ".$field_query.",DATE_FORMAT(TRANSACTION_DATE_CREATED, '%Y-%m-%d %h:%i:%s %p') as TRANSACTION_DATE_CREATED FROM transaction ".$sql_conds." ORDER BY TRANSACTION_DATE_CREATED ASC";
