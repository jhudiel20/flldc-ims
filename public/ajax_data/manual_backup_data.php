...
$sorters =array();
$orderby ="backup_logs.DATE_CREATED DESC";
$sql_where="";
$sql_conds="";
...
$dbfield = array('backup_logs.ID','backup_logs.USER_ID','backup_logs.BACKUP_NAME','backup_logs.DATE_CREATED','FNAME','MNAME','LNAME'); // need iset based sa table columns na need sa query
$db_orig = array('ID','USER_ID','BACKUP_NAME','DATE_CREATED'); //gamit sa filter  tabulator
...
//array_push($sql_where_array, " RETIREMENT_STATUS = 0 "); //set default WHERE CONDITION HERE
...
//total query counter 
$total_query = 0;
$field_query ='COUNT(USER_ID) as count'; // baguhin based sa need
$sql_conds = (empty($sql_where)) ? '' : 'WHERE '.$sql_where;
$default_query ="SELECT ".$field_query." FROM  ".$sql_conds;
...
$default_query ="SELECT ".$field_query." FROM backup_logs left join ``.user_account on backup_logs.user_id = user_account.ID ORDER BY ".$orderby;
